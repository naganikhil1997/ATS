    ...
    def match_resumes(self, job_description):
        try:
            ...
            for resume in resumes:
                ...
                file_path = resume.get("file_path", None)
                ...
                if match_score >= 70:
                    chat_session = model.start_chat(history=[])
                    prompt = (
                        f"You are a skilled ATS (Applicant Tracking System) scanner with a deep understanding of job roles. "
                        f"Evaluate the resume against the provided job description and give only the percentage score "
                        f"that indicates how well the resume matches the job description. "
                        f"The output should be in the format 'Score: X%', where X is the percentage match.\n\n"
                        f"Job Description:\n{job_description}\n\nResume:\n{resume_text}"
                    )
                    ...
                    matched_resumes.append({
                        ...
                        "file_path": file_path,
                        "score": match_score,
                        "ai_feedback": ai_feedback.strip()  # Expected to be in "Score: X%" format
                    })
...
def ai_resume_upload_query():
    try:
        # Ensure 'query' is present in the request form
        ...
        if 'file' not in request.files:
            return jsonify({"status": "error", "message": "Resume file(s) are required"}), 400
        ...
        all_resumes_text = []
        pdf_processor = PDFProcessor(db_uri, db_name)  # Create a PDFProcessor instance
        ...
        for file in files:
            ...
            all_resumes_text.append(f"Resume: {file.filename}\n{resume_text}\n")

            # Store each resume in MongoDB
            processor.process_pdf(file.filename, pdf_file)

        # Combine all resume texts for AI query
        resumes_combined_text = '\n'.join(all_resumes_text)

        chat_session = model.start_chat(history=[])
        prompt = f"The HR manager has the following question: {user_query}. Here are the resumes:\n\n{resumes_combined_text}\n\nPlease answer the HR manager's query based on the resume information."
        response = chat_session.send_message(prompt)
        ai_response = response.text
...
def evaluate_job_posting():
    try:
        ...
        for resume in resumes:
            ...
            job_keywords = set(job_full_description.split())
            resume_keywords = set(resume_text.split())
            ...
            if match_score >= 70:
                ...
                html_feedback = get_html(ai_feedback, simple=True)
                
                matched_resumes.append({
                    "file_name": file_name,
                    "match_score": match_score,
                    "ai_feedback": html_feedback.strip()
                })
